...
nc=netcdf(child_grd);
np = netcdf(parent_grd);
% Find grid places of child in parent's grid
lon_child = nc{'lon_rho'}(1,:);
lat_child = nc{'lat_rho'}(:,1);
lon_par = np{'lon_rho'}(1,:);
lat_par = np{'lat_rho'}(:,1);
if (lon_child(1)<lon_par(1) || lon_child(end)>lon_par(end))
    error('Child longitudes are outside parent')
end
if (lat_child(1)<lat_par(1) || lat_child(end)>lat_par(end))
    error('Child latitudes are outside parent')
end
imin = floor(interp1(lon_par, 1:numel(lon_par), lon_child(1)));
imax = floor(interp1(lon_par, 1:numel(lon_par), lon_child(end)));
jmin = floor(interp1(lat_par, 1:numel(lat_par), lat_child(1)));
jmax = floor(interp1(lat_par, 1:numel(lat_par), lat_child(end)));
close(nc);
Lp=length(np('xi_rho'));
Mp=length(np('eta_rho'));
if extrapmask==1
  mask=np{'mask_rho'}(:);
else
  mask=[];
...
for tindex=1:length(thetime)
  ...
  disp('hbbl...')
  interpvar3d(np,ncrst,igrd_r,jgrd_r,ichildgrd_r,jchildgrd_r,'hbbl',mask,tindex)
  disp('rufrc...')
  interpvar3d(np,ncrst,igrd_u,jgrd_u,ichildgrd_u,jchildgrd_u,'rufrc',mask,tindex)
  disp('rvfrc...')
  interpvar3d(np,ncrst,igrd_v,jgrd_v,ichildgrd_v,jchildgrd_v,'rvfrc',mask,tindex)
  disp('dRdx...')
  interpvar4d(np,ncrst,igrd_u,jgrd_u,ichildgrd_u,jchildgrd_u,'dRdx',mask,tindex,N)
  disp('dRde...')
  interpvar4d(np,ncrst,igrd_v,jgrd_v,ichildgrd_v,jchildgrd_v,'dRde',mask,tindex,N)
