function interp_OGCM2(OGCM_dir,OGCM_prefix,year,month,Roa,interp_method,...
...
zeta=ext_data_OGCM2(lonT,latT,nc{'ssh'},tin,lon,lat,1,Roa,interp_method,defaultvalue);
%
if ~isempty(nc_clm)
%
% Read and extrapole the 2D variables
%
  u2d=ext_data_OGCM2(lonU,latU,nc{'ubar'},tin,lon,lat,1,Roa,interp_method,defaultvalue);
  v2d=ext_data_OGCM2(lonV,latV,nc{'vbar'},tin,lon,lat,1,Roa,interp_method,defaultvalue);
...
% 
  ...
  u=zeros(NZ,M,L-1);
  v=zeros(NZ,M-1,L);
  % Prefetch data prior to run parallel
  nc_u = nc{'u'}(:);
  nc_v = nc{'v'}(:);
  nc_temp = nc{'temp'}(:);
  nc_salt = nc{'salt'}(:);
  ...
  parfor k=1:NZ
    ...
    u2d=ext_data_OGCM2(lonU,latU,nc_u,tin,lon,lat,k,Roa,interp_method,defaultvalue);
    v2d=ext_data_OGCM2(lonV,latV,nc_v,tin,lon,lat,k,Roa,interp_method,defaultvalue);
    u(k,:,:)=rho2u_2d(u2d.*cosa+v2d.*sina);
    v(k,:,:)=rho2v_2d(v2d.*cosa-u2d.*sina);
    temp(k,:,:)=ext_data_OGCM2(lonT,latT,nc_temp,tin,lon,lat,k,Roa,interp_method,defaultvalue);
    % Salt needs special value
    salt(k,:,:)=ext_data_OGCM2(lonT,latT,nc_salt,tin,lon,lat,k,Roa,interp_method,34.6);
  end
end
...
% Read and extrapole the 2D variables
  if obc(1)==1
    ...
    ubar1d=ext_data_OGCM2(lonU,latU,nc{'ubar'},tin,...
	   squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),1,Roa,interp_method,defaultvalue);
    vbar1d=ext_data_OGCM2(lonV,latV,nc{'vbar'},tin,...
	   squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),1,Roa,interp_method,defaultvalue);
  ...
  if obc(2)==1
    ...
    ubar1d=ext_data_OGCM2(lonU,latU,nc{'ubar'},tin,...
	   squeeze(lon(:,end-1:end)),squeeze(lat(:,end-1:end)),1,Roa,interp_method,defaultvalue);
    vbar1d=ext_data_OGCM2(lonV,latV,nc{'vbar'},tin,...
	   squeeze(lon(:,end-1:end)),squeeze(lat(:,end-1:end)),1,Roa,interp_method,defaultvalue);
...
% Prefetch data prior to run parallel
  ...
  parfor k=1:NZ
    ...
    if obc(1)==1 % Southern boundary
      t1d=squeeze(ext_data_OGCM2(lonT,latT,nc_temp,tin,...
                  squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),...
                  k,Roa,interp_method,defaultvalue));
      s1d=squeeze(ext_data_OGCM2(lonT,latT,nc_salt,tin,...
                  squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),...
                  k,Roa,interp_method,defaultvalue));
      temp_south(k,:)=squeeze(t1d(1,:));
      salt_south(k,:)=squeeze(s1d(1,:));
      u1d=ext_data_OGCM2(lonU,latU,nc_u,tin,...
                  squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),...
                  k,Roa,interp_method,defaultvalue);
      v1d=ext_data_OGCM2(lonV,latV,nc_v,tin,...
                  squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),...
                  k,Roa,interp_method,defaultvalue);
